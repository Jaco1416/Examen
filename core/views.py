...
def index(request):
    ...
    if request.method == 'POST':
        ...
        usuarioprod = request.POST.get('txtUser')
        ...
        if Carrito.objects.filter(Q(codigo=codigoprod) & Q(usuario=usuarioprod)  & Q(nombre=nombre)).exists(): 
            ...
            prod.usuario = request.POST.get('txtUser')
            prod.save()
...
@login_required
def cart(request):
    carritoAll = Carrito.objects.filter(usuario = request.user) # SELECT * FROM producto
...
def historial(request):
    detalleCompraAll = DetallesCompra.objects.all()
    carritoAll = Carrito.objects.filter(usuario=request.user)
    datos = {
        'ListaDetallecompra' : detalleCompraAll 
    }
